...
public class GaussianKernelLogisticRegression extends KernelLogisticRegression {
	...
	public void train(List<Instance> instances) {
		...
		this.sigma = Learn.gaussian_kernel_sigma;
		...
		for(int i=0; i<instances.size(); i++) {
			...
			for(int j=0; j<instances.size(); j++) {
				...
				RealVector x = xi.subtract(xj);
				gramMatrix[i][j] = Math.pow(Math.E, -(Math.pow(x.getNorm(), 2))/(2*Math.pow(sigma, 2)));
			}
		}
	...
	public Label predict(Instance instance) {
		...
		for(int j=0; j<instances.size(); j++) {
			...
			RealVector xj = new ArrayRealVector(xj_array);
			RealVector x = xi.subtract(xj);
			z += a_j*(Math.pow(Math.E, -(Math.pow(x.getNorm(), 2))/(2*Math.pow(sigma, 2))));
		}

...
public class Learn {
	...
	static String kernel = "linear_kernel";
	static double gradient_ascent_learning_rate = 0.01;
	static double polynomial_kernel_exponent = 2;
	static double gaussian_kernel_sigma = 1;
	static int gradient_ascent_training_iterations = 5;
	...
	public static void main(String[] args) throws IOException {
		...
		//String data = "data\\data\\speech.dev";
		String predictions_file = CommandLineUtilities.getOptionValue("predictions_file");
		//String predictions_file = "speechPreds";
		String algorithm = CommandLineUtilities.getOptionValue("algorithm");
		//String algorithm = "lambda_means";
		String model_file = CommandLineUtilities.getOptionValue("model_file");
		//String model_file = "speechTrain";
		String task = CommandLineUtilities.getOptionValue("task"); // classification vs. regression
		//String task = "classification";
		
		if (CommandLineUtilities.hasArg("lambda")) lambda = CommandLineUtilities.getOptionValueAsFloat("lambda");
		if (CommandLineUtilities.hasArg("online_learning_rate")) online_learning_rate = CommandLineUtilities.getOptionValueAsFloat("online_learning_rate");
		if (CommandLineUtilities.hasArg("kernel")) kernel = CommandLineUtilities.getOptionValue("kernel");
		if (CommandLineUtilities.hasArg("polynomial_kernel_exponent")) polynomial_kernel_exponent = CommandLineUtilities.getOptionValueAsFloat("polynomial_kernel_exponent");
		if (CommandLineUtilities.hasArg("gaussian_kernel_sigma")) gaussian_kernel_sigma = CommandLineUtilities.getOptionValueAsFloat("gaussian_kernel_sigma");
		if (CommandLineUtilities.hasArg("gradient_ascent_learning_rate")) gradient_ascent_learning_rate = CommandLineUtilities.getOptionValueAsFloat("gradient_ascent_learning_rate");
		if (CommandLineUtilities.hasArg("gradient_ascent_training_iterations")) gradient_ascent_training_iterations = CommandLineUtilities.getOptionValueAsInt("gradient_ascent_training_iterations");
		if (CommandLineUtilities.hasArg("cluster_lambda")) cluster_lambda = CommandLineUtilities.getOptionValueAsFloat("cluster_lambda");
		if (CommandLineUtilities.hasArg("clustering_training_iterations")) clustering_training_iterations = CommandLineUtilities.getOptionValueAsInt("clustering_training_iterations");
	...
	private static Predictor train(List<Instance> instances, String algorithm) {
		...
		if(algorithm.equalsIgnoreCase("majority")) {
			//if algorithm to be used is majority classifier
			predictor = new MajorityClassifier();
		} else if(algorithm.equalsIgnoreCase("even_odd")) {
			//if algorithm to be used is even odd classifier
		...
		} else if(algorithm.equalsIgnoreCase("kernel_logistic_regression")) {
			if(kernel.equalsIgnoreCase("linear_kernel")) {
				...
				System.out.println("kernel logistic regression type is invalid");
				System.exit(0);
			}
			...
			System.out.println("Algorithm name is not valid");
			System.exit(0);
		}
		//train the appropriate predictor
		predictor.train(instances);
	    // TODO Evaluate the model
		//accuracy evaluator object
		/*
